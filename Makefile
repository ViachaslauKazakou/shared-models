...
backup-remote: ## Создать резервную копию удаленной БД
	@echo "$(GREEN)Opening backup utility for remote database...$(NC)"
	./scripts/backup-remote-db.sh

backup-list: ## Показать список локальных бэкапов
	@echo "$(GREEN)Local backups:$(NC)"
	@ls -lh backups/backup_* 2>/dev/null || echo "No backups found"
	@echo ""
	@du -sh backups 2>/dev/null || echo "Backup directory does not exist"
