...
async def test_cascade_deletions():
    ...
    try:
        ...
        async with async_session() as session:
            ...
            # Создание темы
            topic = Topic(title="Test Topic", description="Test description", user_id=user.id)
            session.add(topic)
            await session.commit()
            ...
            # Создание сообщения
            message = Message(
                content="Test message content", author_name=user.username, topic_id=topic.id, user_id=user.id
            )
            session.add(message)
            await session.commit()
            ...
            message_embeddings_count = len((await session.execute(select(MessageEmbedding))).scalars().all())

            user_examples_count = len((await session.execute(select(UserMessageExample))).scalars().all())
            ...
            remaining_message_embeddings = (
                (await session.execute(select(MessageEmbedding).where(MessageEmbedding.message_id.isnot(None))))
                .scalars()
                .all()
            )
